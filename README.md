...
背景與設計理由見 [`backend/data/schema.md`](backend/data/schema.md)——那份文件是這個專案的「資料契約」：
...
中間確認過畫面設計、資料庫格式、多張照片、修改刪除的作法，最後才落地成這個 repo。
過程紀錄與決策理由不在程式碼裡，在當時的對話裡；程式碼裡只留下最後的結果與註解說明「為什麼」。
...
```
backend/
  server.js          Express app：靜態檔案 + REST API
  lib/
...
frontend/
  index.html           全部畫面（儀表板/列表/日曆/新增/回收桶），fetch 這個 repo 的 API
scripts/
  migrate.js            一次性搬遷腳本
test/                   node:test，見下方
